...
export default registerComponent("Petloop Summary Daily Walk in", {
  type: "none",
  icon: "IconX",
  group: "input",
  defaultProps: {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await client.project.axios({
          ...
          url: `${url}/v1/rawsql`,
          data: {
            query: `SELECT COUNT(id) as total_transaction, SUM(grand_total) as total_income, DATE(date) as trans_date FROM bills WHERE bill_service_place_id = '${servicePlaceId}' AND DATE(date) = '${year}-${month}-${day}' AND status = 'Paid' GROUP BY DATE(date)`
          }
        })
